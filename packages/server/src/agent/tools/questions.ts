...
const askUserQuestionsDescription = `Ask the user a bounded batch of independent multiple-choice questions when the automation builder needs several setup choices before it can proceed. Use this only when all questions are ready together; otherwise use AskUserQuestion. For a new automation that needs setup, first let the user describe the automation in their own words; after that, execution mode must be the first bounded choice unless the user already selected it. Do not add an execution-mode question for a simple reminder that can be created directly. Provide ${WEB_CHAT_QUESTION_BATCH_MIN_QUESTIONS}–${WEB_CHAT_QUESTION_BATCH_MAX_QUESTIONS} questions, exactly ${WEB_CHAT_QUESTION_MIN_OPTIONS}–${WEB_CHAT_QUESTION_MAX_OPTIONS} options per question, stable question and option IDs, and no duplicate IDs. This ends the current run and creates a pending chat-surface interaction; after calling it, stop and wait for the user's selections.`;
