...
export function createSearchChatHistoryTool(deps: SketchMcpDeps) {
  return tool(
    ...
    "Search persisted messages in the current chat conversation by keyword, topic, name, decision, project, phrase, or older chat reference. Use this as the first tool for targeted chat-history discovery, even when only some missed messages were inlined. Use ReadChatHistory only for chronological paging or reading around a known message id.",
    {
      ...
      scope: z
        ...
        .optional()
        .describe(
          "Search the whole current conversation or only the active Slack thread. Defaults to current_thread when a Slack thread is active, otherwise conversation.",
        ),
    ...
    async ({ query, scope, afterMessageId, beforeMessageId, limit, includeBotMessages }) => {
      const conversationId = deps.conversationContext?.conversationId;
      if (!conversationId || !deps.conversationRepo) {
