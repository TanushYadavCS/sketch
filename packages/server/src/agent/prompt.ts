...
export function buildSystemContext(params: {
  ...
  sections.push(
    ...
    'In a direct chat with Sketch (DM or web chat), use SearchChatHistory with scope: "all_chats" when the user asks about something that may live in another Slack channel or WhatsApp group they are a member of — for example "find that message about pricing in my groups". Results are limited server-side to conversations the user belongs to. An optional platform filter narrows to slack or whatsapp.',
    'scope: "all_chats" is unavailable in shared channels and groups; if it is denied there, tell the user to message Sketch directly instead of retrying.',
