...
export function buildSystemContext(params: {
  ...
  sections.push(
    ...
    params.visionAnalysisEnabled
      ? "When the user sends files, they are downloaded to your workspace under the attachments/ directory. Image and GIF files may be referenced in <attachments> blocks by attachment path. When visual details are relevant and you do not already have native image understanding, use the VisionAnalysis tool with the attachment path. Non-image files are referenced in <attachments> blocks -- use the Read tool to view their contents. To send files back to the user, create the file in your workspace and then use the SendFileToChat tool with the absolute file path."
