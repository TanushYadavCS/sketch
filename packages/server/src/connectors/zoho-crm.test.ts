...
describe("Zoho CRM connector", () => {
  ...
  it("syncs paginated standard modules with If-Modified-Since, person seeds, and parent links", async () => {
    ...
    const connector = createZohoCrmConnector();
    const people: Array<{ name: string; email?: string; sourceId: string }> = [];
    const items = [];
    ...
    for await (const item of connector.sync({
      ...
      onPersonSeed: async (seed) => {
        people.push({ name: seed.name, email: seed.email, sourceId: seed.sourceId });
      },
    })) {
      items.push(item);
    }
    ...
    expect(items[2].parentEntities).toEqual([
      { source: "zoho_crm", sourceId: "Accounts:a1", contextSnippet: "Deal account" },
    ]);
    expect(people).toEqual(
      expect.arrayContaining([
        { name: "Owner One", email: "owner@sketch.test", sourceId: "user:u1" },
        { name: "Jane Buyer", email: "jane@acme.test", sourceId: "Contacts:c1" },
        { name: "Jane Buyer", email: undefined, sourceId: "Contacts:c1" },
      ]),
    );
