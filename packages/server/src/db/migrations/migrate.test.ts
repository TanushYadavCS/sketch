 ...
 *
 * Uses a fresh in-memory SQLite database and runs all 030 migrations through the
 ...
 * with migrations 001-018 already applied can be upgraded with only 019-030.
 */
...
describe("runMigrations — full sequence", () => {
  ...
  it("records all 30 migration entries in the kysely_migration table", async () => {
...
describe("runMigrations — incremental upgrade", () => {
  ...
  it("applies only 019-030 when 001-018 are already present", async () => {
    ...
    // Running again should be a no-op (all 30 already applied)
    await runMigrations(db);
