...
describe("runMigrations — full sequence", () => {
  ...
  it("records all 43 migration entries in the kysely_migration table", async () => {
  ...
  it("records migrations with the correct names in order", async () => {
    ...
    expect(names[41]).toBe("042-entity-mentions-composite-index");
    expect(names[42]).toBe("043-users-timezone");
  });
...
describe("runMigrations — incremental upgrade", () => {
  ...
  it("applies only pending migrations when 001-018 are already present", async () => {
    ...
    // Running again should be a no-op (all 43 already applied)
    await runMigrations(db);
