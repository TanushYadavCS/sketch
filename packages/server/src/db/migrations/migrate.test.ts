 ...
 *
 * Uses a fresh in-memory SQLite database and runs all 031 migrations through the
...
describe("runMigrations — full sequence", () => {
  ...
  it("records migrations with the correct names in order", async () => {
    ...
    expect(names[25]).toBe("026-entities");
    expect(names[26]).toBe("027-drop-tags-from-fts");
    expect(names[27]).toBe("028-smart-enrichment");
    expect(names[28]).toBe("029-org-context");
    expect(names[29]).toBe("030-browse-cache");
    expect(names[30]).toBe("031-sync-interval");
  });
