...
describe("AddIntegrationDialog", () => {
  ...
  it("returns the verified native Canvas connection after OAuth succeeds", async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime });
    const onSuccess = vi.fn();
    const popup = { closed: false, close: vi.fn(), location: { href: "" } };
    ...
    server.use(
      ...
      http.post("/api/mcp-servers/provider-1/connections/intents", () =>
        HttpResponse.json({
          app: { id: "google-gmail-oauth", name: "Gmail", description: "Email" },
          redirectUrl: "https://example.com/oauth",
        }),
      ),
      http.get("/api/mcp-servers/provider-1/connections", () =>
        HttpResponse.json({
          connections: [
            {
              id: "secrets:user-1:google:google-gmail-oauth",
              providerId: "provider-1",
              source: "canvas_user_secrets",
    ...
    await waitFor(() =>
      expect(onSuccess).toHaveBeenCalledWith(
        expect.objectContaining({ id: "google-gmail-oauth", connectionId: "secrets:user-1:google:google-gmail-oauth" }),
